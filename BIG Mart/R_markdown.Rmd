...
* Mathematically, an **outlier** is usually defined as an observation more than three standard deviations from the mean (although sometimes you'll see 2.5 or 2 as well)
```{r warning=FALSE}
#Here, we will use scores from outliers package to quickly calculate the z-score for every value in a specific column of our data frame.

library(outliers)
outlier_Item_Outlet_Sales = scores(Train$Item_Outlet_Sales)
Train$outlier_Item_Outlet_Sales <- outlier_Item_Outlet_Sales > 3 | outlier_Item_Outlet_Sales < -3
  
Train$outlier_Item_Outlet_Sales[outlier_Item_Outlet_Sales == FALSE] == 0

ggplot(Train, aes(y=Item_Outlet_Sales,x=Item_MRP,fill = outlier_Item_Outlet_Sales)) + 
  geom_point()
```

```{r}
ggplot(Train, aes(Item_Outlet_Sales)) +
  geom_histogram(binwidth = 100, fill = "cadet blue4") + 
    scale_x_continuous(breaks = seq(0,14000, by =1000)) +
